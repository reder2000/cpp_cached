	...
	{ "dbname", "cppcached" }, { "password","qustrat" }, { "user","postgres" } })
{
	...
	void PostgresCache::create_db()
	{
		try
		{
			...
			w.exec0(
				...
				" value BYTEA)");
			w.exec0(
				"CREATE INDEX IF NOT EXISTS Cache_key ON"
				" Cache (key)");
			w.exec0(
		...
		_connection.prepare("get", "SELECT value FROM CACHE where key = $1");
	}
	//
	...
	bool PostgresCache::has(const std::string& key)
	{
		...
		try
		{
			{
				pqxx::work w(_connection);
				auto res = w.exec_params("SELECT key FROM cache WHERE key = $1", key);
				auto s = res.size();
	...
	{
		pqxx::work w(_connection);
		w.exec_params0("DELETE FROM cache where key=$1", key);
		w.commit();
	}
	//
