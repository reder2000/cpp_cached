...
void SqliteSingleCache::create_db()
{
	try
	{
		...
		try_exec_retry(db,"CREATE INDEX IF NOT EXISTS Cache_expire_time ON"
			" Cache (expire_time)",1);
		try_exec_retry(db,"CREATE INDEX IF NOT EXISTS Cache_access_time ON"
			" Cache (access_time)",1);
