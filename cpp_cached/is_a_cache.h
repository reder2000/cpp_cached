...
concept is_a_cache =
    ...
    requires(T) { typename T::template return_type<int>; };


template <is_a_cache cache, class F>
typename cache::template return_type<std::decay_t<std::invoke_result_t<F>>>
cache_get(cache& a_cache, const std::string& key, F callback)
{
  using T = std::decay_t<std::invoke_result_t<F>>;
  if (a_cache.has(key)) return a_cache.template get<T>(key);
  a_cache.set(key, callback());
  return a_cache.template get<T>(key);
}
